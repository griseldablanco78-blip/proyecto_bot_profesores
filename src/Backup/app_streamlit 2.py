...
# Archivos locales para solicitudes y concesiones
...
def save_json_safe(p: Path, obj):
    p.write_text(json.dumps(obj, ensure_ascii=False, indent=2), encoding="utf-8")

# cargar almacenamiento local (si existe)
access_reqs = load_json_safe(ACCESS_REQS)
access_grants = load_json_safe(ACCESS_GRANTS)

# Aseguramos variables existentes (si ya las usás arriba)
try:
    requester_email  # noqa: F821
except NameError:
    requester_email = ""
...
# intentar extraer email de la service account si se subió la JSON (sa_path viene de la parte de subida)
service_account_email = None
try:
    if 'sa_path' in globals() and sa_path and os.path.exists(sa_path):
...
# Inputs visibles en sidebar (reutiliza los que ya podrías tener)
requester_email = st.sidebar.text_input("Tu correo (quien solicita acceso)", value=requester_email)
sheet_url = st.sidebar.text_input("URL de la hoja de cálculo de Google (opcional)", value=sheet_url)
...
if st.sidebar.button("Generar solicitud de acceso (crear token)", key="gen_token_btn"):
    if not requester_email or not sheet_url:
        ...
        st.sidebar.success("Token generado. Abrí el correo o copiá el cuerpo y envialo al propietario.")
...
if st.sidebar.button("Confirmar token", key="confirm_token_btn"):
    if not entered_token:
        ...
        found = None
        for k, v in list(access_reqs.items()):
        ...
        if not found:
            ...
            access_grants[grant_key] = {
                "requester": rec["requester"],
                "sheet_url": rec["sheet_url"],
                "sa_email": rec.get("sa_email"),
                "granted_ts": datetime.utcnow().isoformat() + "Z"
            }
            save_json_safe(ACCESS_GRANTS, access_grants)
            # eliminar la solicitud (one-time)
            try:
                del access_reqs[key]
            except KeyError:
                pass
            save_json_safe(ACCESS_REQS, access_reqs)
            st.sidebar.success("Autorización confirmada localmente. La app la marcará como permitida para este usuario y Sheet.")
...
# Nota: antes de intentar leer el Google Sheet, comprobá esta autorización local:
# if sheet_url and sa_path and not is_authorized_locally(requester_email, sheet_url):
#     st.sidebar.warning("La app no tiene autorización local. Generá la solicitud y pedí al propietario que comparta el Sheet con la cuenta de servicio y devuelva el TOKEN.")


# ------------------------------------------------------------------------
# Preparar campos (materia, año, unidad)
# ------------------------------------------------------------------------
# --- Asegurar que sheets_dict exista y cargar Excel local si está vacío ---
# Si sanitize_dataframe no está definida, definimos una versión mínima
if 'sanitize_dataframe' not in globals():
    def sanitize_dataframe(df):
        try:
            return df.fillna("").applymap(lambda v: "" if (isinstance(v,str) and v.strip().startswith("=")) else v)
        except Exception:
            return df.fillna("")

# Garantizar la variable sheets_dict
if 'sheets_dict' not in globals():
    sheets_dict = {}

# Si sheets_dict está vacío, intentar cargar automáticamente el primer Excel en /data
if not sheets_dict:
    data_folder = Path("data")
    excels = list(data_folder.glob("*.xlsx"))
    if excels:
        try:
            loaded = pd.read_excel(str(excels[0]), sheet_name=None, engine="openpyxl")
            # Sanitizamos (removemos fórmulas y NaN)
            sheets_dict = {k: sanitize_dataframe(v) for k, v in loaded.items()}
            st.sidebar.success(f"Cargadas {len(sheets_dict)} hojas desde Excel local (autocarga).")
        except Exception as e:
            st.sidebar.error("Error leyendo Excel local automáticamente: " + str(e))

# Finalmente extraemos candidates (siempre con un dict, aunque vacío)
subjects, years, units = extract_candidates_from_sheets(sheets_dict or {})
